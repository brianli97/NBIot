	...
	{ 
        for (i = 0;i < user_cmd_count; i++)
        {
            if (user_cmd[i].sta == RCV_SUCC)
            {
                ...
                if (!strncmp(user_cmd[i].buff, "+CHTTPNMIC", strlen("+CHTTPNMIC")))
                {
                    /* +CHTTPNMIC: 4,0,34,34,7b0a2020226f726967696e223a20223232332e3130342e3235352e313239220a7d0 */
                    str_len = strlen(user_cmd[i].buff);
                    *(user_cmd[i].buff + str_len - 1) = '\0';
                    *(user_cmd[i].buff + str_len - 2) = '\0';
                    str_len = strlen((user_cmd[i].buff + strlen("+CHTTPNMIC: 1,0,34,34,")));
                    printf("str_len:%d\n", str_len);
					/* 注意：这里是为了省事         实际情况需要把16进制的长度和起始位置需要根据'+CHTTPNMIC: 1,0,34,34,'计算出来 */
                    hex_to_str((user_cmd[i].buff + strlen("+CHTTPNMIC: 1,0,34,34,")), buff, str_len / 2);
